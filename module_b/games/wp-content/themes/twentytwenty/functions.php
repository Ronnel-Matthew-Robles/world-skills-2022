...
// create function
function add_aggregator_to_admin_menu() {
	add_menu_page(
		'News Aggregator',
		'News Aggregator',
		'manage_options',
...
add_action('admin_menu', 'add_aggregator_to_admin_menu');
...
function display_aggregator() {
	...
	foreach ($json_urls as $json_url) {
		...
		if (!empty($news)) {
			...
			$total_posts_per_site[get_site_name($json_url)] = $posts_count;
		}
	}

	if (isset($_POST['q'])) {
		$query = $_POST['q'];
		$filtered_news = array_filter($all_news, function($news_post) use ($query) {
			return stripos($news_post['title']['rendered'], $query) !== false || stripos($news_post['excerpt']['rendered'], $query) !== false;
		});
...
if (!empty($all_news)) {
	foreach ($all_news as $news_post){
		?>
<h3><?php echo $news_post['title']['rendered'] ?></h3>
<p><?php echo $news_post['excerpt']['rendered'] ?></p>
...
function get_site_name($site_name) {
	$site_name = str_replace('http://localhost/module_b/', '', $site_name);
...
function add_faq_to_admin_dashboard() {
	add_menu_page(
		'FAQ',
		'FAQ',
		'read',
...
add_action('admin_menu', 'add_faq_to_admin_dashboard');
...
<style>
h1 a {
    background-image: url(http://localhost/XX_module_b/games/wp-content/uploads/2023/03/wordpress-fluid-typography.png) !important;
}
</style>
<?php
}
...
function register_mini_banners_post_type() {
	$labels = array(
		'name' => 'Mini Banners',
		'signular_name' => 'Mini Banner',
		'add_new' => 'Add New',
		'add_new_item' => 'Add New Mini Banner',
...
add_action('init', 'register_mini_banners_post_type');
